...
namespace Text
{
    ...
    void DrawGlyph(FT_Face face, GlyphMetrics glyphMetrics, int textureSize, int glyphSize, int padding, float distanceMappingRange, int renderFlags, Buffer<RGBA32Pixel> *refTexture)
    {
        ...
        float translate_x = (padding - bounds.l * scale) / scale;
        float translate_y = (padding - bounds.b * scale) / scale;
        msdfgen::Vector2 translate(translate_x, translate_y);
        msdfgen::Bitmap<float, 4> tempBitmap(glyphMetrics.atlas_width_px, glyphMetrics.atlas_height_px);
        ...
        for (int dx = 0; dx < glyphMetrics.atlas_width_px; ++dx)
        {
            ...
            {
                int dest_x = glyphMetrics.atlas_x_px + dx;
                int dest_y = glyphMetrics.atlas_y_px + dy;
                int src_x = dx;
                int src_y = dy;
                if (dest_x >= 0 && dest_x < textureSize && dest_y >= 0 && dest_y < textureSize)
                {
                    auto dest = refTexture->Data() + dest_y * textureSize + dest_x;
